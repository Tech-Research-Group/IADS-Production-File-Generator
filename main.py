"""IADS PRODUCTION FILE GENERATOR"""
...
def open_wip_dir() -> None:
    ...
    STYLESHEET_EDITOR = '<?xml-stylesheet href="40051D_7_0.fos" type="text/x-fosi" media="editor" alternate="yes" title="Editor Stylesheet"?>'
    STYLESHEET_PRINT = '<?xml-stylesheet href="cstyle-v7_0_2/xslfo-main-v7_0.xsl" type="text/xsl" media="pdf, print" alternate="yes" title="PDF Stylesheet"?>'

    DOCTYPE_OPEN = '<!DOCTYPE production PUBLIC "-//USA-DOD//DTD -1/2D TM Assembly REV D 7.0 20220130//EN" "40051D_7_0.dtd" ['
    DOCTYPE_CLOSE = "]>"
    production_tag = f'<production xmlns:xi="http://www.w3.org/2001/XInclude" date="(DAY) (MONTH) (YEAR)" chnglevel="0" chngdate="{get_date()}" pin="XX-XXXX-XXX-XX">'
    ...
    if filenames := filedialog.askopenfilenames():
        for path in filenames:
            ...
            if "entitydeclaration" in filename.lower():
                ...
                "front cover" in filename.lower()
                or "title page" in filename.lower()
                or "frontmatter" in filename.lower()
            ):
                tag = "frntcover"
            ...
            # OPERATOR INSTRUCTIONS
                ...
                "O00" in filename
                and "unusual" not in filename.lower()
                and "start" not in filename.lower()
                and "end" not in filename.lower()
                and "chap" not in filename.lower()
            ):
                if usual_entities == "":
                    tag = "opusualwp"
            ...
            # TROUBLESHOOTING
                ...
                "TS" in filename or "troubleshooting" in filename.lower()
            ) and "intro" in filename.lower():
                tag = "tsintrowp"
                with open(path, "r", encoding="utf-8") as _f:
                    for line in _f:
                        line = line.lstrip("\t")
                ...
                "T000" in filename
                and "intro" not in filename.lower()
                and "index" not in filename.lower()
                and "start" not in filename.lower()
                and "end" not in filename.lower()
                and "chap" not in filename.lower()
            ):
            ...
            # PREVENTIVE MAINTENANCE CHECKS AND SERVICES (PMCS)
                ...
                "M00" in filename and "PMCS" in filename and "intro" in filename.lower()
            ):
                tag = "pmcsintrowp"
                with open(path, "r", encoding="utf-8") as _f:
                    for line in _f:
                ...
                "M000" in filename
                and "PMCS" in filename
                and "daily" in filename.lower()
                or "weekly" in filename.lower()
                or "monthly" in filename.lower()
                or "quarterly" in filename.lower()
                or "semi-annually" in filename.lower()
                or "annually" in filename.lower()
                or "before" in filename.lower()
                or "during" in filename.lower()
                or "after" in filename.lower()
            ):
                tag = get_wp_code(filename)
                mim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
            elif (
                ("M001" in filename or "M002" in filename)
                and "PMCS" in filename
                and "daily" in filename.lower()
                or "weekly" in filename.lower()
                or "monthly" in filename.lower()
            ...
            # DESTRUCTION OF EQUIPMENT TO PREVENT ENEMY USE
                ...
                "D00" in filename
                and "intro" in filename.lower()
                and "destruct" in filename.lower()
            ):
            ...
            # REPAIR PARTS AND SPECIAL TOOLS LIST (RPSTL)
                ...
                "R00" in filename
                and "intro" not in filename.lower()
                and "nsn" not in filename.lower()
                and "pn" not in filename.lower()
                and "bulk" not in filename.lower()
                and "partnumber" not in filename.lower()
                and "index" not in filename.lower()
                and "start" not in filename.lower()
                and "end" not in filename.lower()
            ):
                ...
                "R00" in filename
                and "bulk" in filename.lower()
                and "item" in filename.lower()
            ):
                tag = "bulk_itemswp"
                pim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
            elif (
                "R00" in filename and "NSN" in filename and "index" in filename.lower()
            ):
                tag = "nsnindxwp"
                pim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
            ...
            # SUPPORTING INFORMATION
                ...
                "S00" in filename
                and "expendable" in filename.lower()
                or "durable" in filename.lower()
                or "EDIL" in filename
            ):
                tag = "explistwp"
                sim_entities += f"&{tag}; "
                ...
                "S00" in filename
                and "AAL" in filename
                or "additional" in filename.lower()
                and "authorization" in filename.lower()
            ):
                tag = "aalwp"
                sim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
                ...
                "S00" in filename
                and "TIL" in filename
                or "tool" in filename.lower()
                and "identification" in filename.lower()
            ):
                tag = "tilwp"
                sim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
                ...
                "S00" in filename
                and "MRP" in filename
                or "mandatory" in filename.lower()
                and "replacement" in filename.lower()
            ):
                tag = "mrplwp"
                sim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
                ...
                "S00" in filename
                and "CSI" in filename
                or "critical" in filename.lower()
                and "safety" in filename.lower()
            ):
                tag = "csi.wp"
                sim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
                ...
                "S00" in filename
                and "additional" in filename.lower()
                and "supporting" in filename.lower()
            ):
                tag = "genwp"
                sim_entities += f"&{tag}; "
                production_xml += f"\t{get_entity_data(tag, filename)}"
    ...
    paper_manual_tag = (
        '<paper.manual pubno="" '
        'maintitl="" '
        'maintlvls="10 | 12 | 20 | dmwr | dmwr-nmwr | nmwr | NA" '
        'rpstl="yes | no | only" '
        'revno="0" '
        'multivolume="yes | no" '
        'fit.paper.size="pocket | logbook | standard | double" '
        'security="uc | cui | c | s | ts">'
    )
    ...
    if opim_entities != "":
        ...
        production_xml += (
            f"\t\t\t{opim_entities}{usual_entities}{unusual_entities}\n\t\t</opim>\n"
        )
...
def save_file():
    ...
    messagebox.showinfo(
        "File Saved", f"The file {filename} has been saved to {filepath}."
    )
...
wip_btn = ttk.Button(
    frame_top, text="WIP FOLDER", command=open_wip_dir, bootstyle="success"
)
